...
if($_POST){
	...
	//SEMAK JAWAPAN BAGI SOALAN MCQ/TF
	if($jenis_soalan==1){
		$_SESSION['jenis_soalan']=1;
		//JAWAPAN BETUL
		$q = "select * from pilihan where nom_soalan =$number 
		and jawapan=1 and idsoalan=$idquestion";
	...
	//SEMAK JAWAPAN BAGI SOALAN FIB
	}else{
		$q = mysqli_query($hubung, "SELECT * FROM pilihan 
		WHERE idsoalan='$idquestion' AND nom_soalan = '$number' AND
